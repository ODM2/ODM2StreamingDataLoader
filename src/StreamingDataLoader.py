...
def main(arguments):
    ...
    if os.path.isdir(arguments.yamlFile):
        arguments.yamlFile += "/*.yaml"

    for filepath in glob.glob(arguments.yamlFile):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', dest="yamlFile",\
        help="YAML configuration file or directory containing .yaml files.", required=True, action="store")
